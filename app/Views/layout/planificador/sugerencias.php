<div class="content-wrapper">
    ...
    <br>
    <hr>
    <p style="text-align: justify;">La función de Búsqueda de Recetas te facilita a encontrar
                ...
                <div class="box box-primary">
                    <form id="search-form">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="ingrediente" name="ingrediente" placeholder="Ej. Pollo, arroz, etc.">
                        </div>
                ...
                success: function(response) {
                    ...
                    if (recetas.length > 0) {
                        ...
                        recetas.forEach(function(receta) {
                            ...
                            resultadosHtml += '<img src="' + receta.recipe.image + '" class="card-img-top" alt="' + receta.recipe.label + '">';
                            resultadosHtml += '<div class="card-body">';
                            resultadosHtml += '<h5 class="card-title">' + receta.recipe.label + '</h5>';
                            resultadosHtml += '<p class="card-text">Calorías: ' + Math.round(receta.recipe.calories) + '</p>';
                            ...
                            if (receta.nutrition.totalNutrients) {
                                ...
                                $.each(receta.nutrition.totalNutrients, function(nutriente, info) {
                                    resultadosHtml += '<li><strong>' + info.label + ':</strong> ' + Math.round(info.quantity, 2) + ' ' + info.unit + '</li>';
                                });
                                resultadosHtml += '</ul>';
                            }

                            resultadosHtml += '<a href="' + receta.recipe.url + '" class="btn btn-primary" target="_blank">Ver Receta</a>';
                            resultadosHtml += '</div></div></div>';
                        });
                        resultadosHtml += '</div>';
                    } else {
                        resultadosHtml = '<p>No se encontraron recetas para el ingrediente seleccionado.</p>';
                    }
